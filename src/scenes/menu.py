...
def _fade_music() -> None:
    ...
    pygame.mixer.Channel(AudioChannel.STATIC).fadeout(250)
...
class Menu(Scene):
    ...
    def enter(self) -> None:
        ...
        if self.should_show_credits:
            ...
            play_sound(AudioChannel.STATIC, a.STATIC, -1)
            self.start_credits()
        else:
        ...
        if self.screen == MenuScreen.MAIN_MENU:
            ...
            if not fade_active(self.fade):
                ...
                if self.ui_index is not None:
                    ...
                    if (
                        t.is_pressed(action_buffer, t.Action.A)
                        or t.is_pressed(action_buffer, t.Action.SELECT)
                        or t.is_pressed(action_buffer, t.Action.START)
                    ):
                        button_activate(element)
                        play_sound(AudioChannel.UI, a.UI_SELECT)
    ...
    def exit(self) -> None:
        stop_music()
        pygame.mixer.Channel(AudioChannel.STATIC).stop()
